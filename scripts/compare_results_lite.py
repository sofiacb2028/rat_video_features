...
def plot_preview(data, out_png="results/csv/comparison_preview.png"):
    """Plot the first 100 points from each dataset to check things are working."""
    if not data:
        print("⚠️ No data to plot (preview)")
        return

    plt.figure(figsize=(10,6))
    for fps, df in data.items():
        preview = df.head(100)
        plt.plot(preview["time_sec"], preview["area"], label=f"{fps} fps", alpha=0.8)

    plt.xlabel("Time (seconds)")
    plt.ylabel("Detected area")
    plt.title("Preview: first 100 frames across FPS settings")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png)
    plt.close()
